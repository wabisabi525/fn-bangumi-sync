...
将飞牛影视的观看记录自动同步至 Bangumi追番，以解决飞牛影视缺乏Webhook主动推送，Bangumi无法同步的功能。

* 🧬 **智能跨季识别**：解决飞牛长篇连载合并与 Bangumi 多季拆分的冲突。
* 🌗 **现代化UI**：自适应网格布局，支持日/夜间模式无缝切换，并提供实时后台日志数据流。
* 🔐 **绝对数据安全**：程序仅读取数据库，零写入操作。
...
### 📌 部署准备
* 拥有 SSH 权限的飞牛 NAS 系统。
* 获取 Bangumi 的 [个人 Access Token]([https://bgm.tv/dev/app](https://next.bgm.tv/demo/access-token))。
...
* **终端显示**：集成 EventSource 推送技术，同步全过程的 API 请求与匹配结果清晰可见。
